...
void PWM0A_Init(uint16_t period, uint16_t duty){
  SYSCTL_RCGCPWM_R |= SYSCTL_RCGCPWM_R0;// 1) activate clock for PWM0
                                        // 2) activate clock for Port K
  SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R9;
                                        // allow time for clock to stabilize
  while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R9) == 0){};
  GPIO_PORTK_AFSEL_R |= 0x10;           // 3) enable alt funct on PK4
  GPIO_PORTK_DEN_R |= 0x10;             //    enable digital I/O on PK4
                                        // 4) configure PK4 as PWM0
  GPIO_PORTK_PCTL_R = (GPIO_PORTK_PCTL_R&0xFFF0FFFF)+0x00060000;
  GPIO_PORTK_AMSEL_R &= ~0x10;          //    disable analog functionality on PK4
                                        // allow time for clock to stabilize
